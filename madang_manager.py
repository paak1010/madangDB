...
# --- 1. DB 접속 정보 설정 (st.secrets 사용) ---
# .streamlit/secrets.toml 파일의 정보를 읽어옵니다.
DB_CONFIG = {
...
def search_user_orders(name: str, conn: pymysql.connections.Connection) -> tuple[pd.DataFrame, Optional[int]]:
    ...
    try:
        with conn.cursor(pymysql.cursors.DictCursor) as cursor:
            ...
            if result:
                orders_df = pd.DataFrame(result, columns=['고객ID', '고객명', '도서명', '판매가', '주문일자'])
...
# DB 연결이 성공한 경우에만 앱의 나머지 부분을 로드
if db_conn:
    st.success("데이터베이스 연결 준비 완료!", icon="✅")
    st.markdown("---")
    ...
    with tab1:
        ...
        name_input = st.text_input("조회할 고객 이름 입력:", key="cust_name_search", placeholder="예: 박지성") 
        
        if st.button("조회하기", key="search_btn"):
            if name_input:
    ...
    with tab2:
        st.header("신규 거래 입력")
        # 1. 고객명 입력 및 ID 찾기
        input_name = st.text_input("거래할 고객 이름 입력:", key="cust_name_trade", placeholder="예: 박지성")
        
        current_custid = None
        if input_name:
            with st.spinner("고객 ID 조회 중..."):
    ...
    st.error("DB 연결이 실패하여 앱을 실행할 수 없습니다. 상단 오류 메시지를 확인하세요.")
